...
BaseConfigObject::BaseConfigObject(const QString &filename, QObject *context)
	: m_filename(filename)
{
	m_saveTimer = new QTimer(context);
	m_saveTimer->setSingleShot(true);
	// cppcheck-suppress pureVirtualCall
	QObject::connect(m_saveTimer, &QTimer::timeout, context, [this](){saveNow();});
	setSaveTimeout(250);

	// cppcheck-suppress pureVirtualCall
	m_appQuitConnection = QObject::connect(qApp, &QCoreApplication::aboutToQuit, context, [this](){saveNow();});
}
